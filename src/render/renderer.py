...
class Renderer:
    ...
    def __center_texts(self, texts: List[str]) -> None:
        offset = self.__config.center_text_offset_y + self.__config.window_padding_y
    ...
    def __render_death(self, content: Content) -> None:
        ...
        self.__center_texts([
            ...
            "{0}: {1}".format(self.__config.translate.level, content.level),
    ...
    def __render_info(self, texts: List[str]):
        length = 0
        for (index, text) in enumerate(texts):
            x = self.__config.window_padding_x + length
            y = self.__config.window_padding_y
            pyxel.text(x, y, text, self.__config.text_color)
            length += len(text) * pyxel.FONT_WIDTH + self.__config.info_text_margin_x

    def coordinate_transform(self, position: Position):
        return Position(
            position.x * self.__config.pixel_per_block + self.__config.window_padding_x,
